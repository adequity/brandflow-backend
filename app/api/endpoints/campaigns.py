...
async def get_campaigns(
    ...
    if viewerId is not None or adminId is not None:
        ...
        if user_role in ['슈퍼 어드민', '슈퍼어드민'] or '슈퍼' in user_role:
            ...
            query = select(Campaign).options(joinedload(Campaign.creator), joinedload(Campaign.client))
        elif user_role in ['대행사 어드민', '대행사어드민'] or ('대행사' in user_role and '어드민' in user_role):
            # 대행사 어드민은 같은 회사 소속 캠페인만
            query = select(Campaign).options(joinedload(Campaign.creator), joinedload(Campaign.client)).join(User, Campaign.creator_id == User.id).where(User.company == current_user.company)
        elif user_role == '직원':
            # 직원은 자신이 생성한 캠페인만 조회 가능
            query = select(Campaign).options(joinedload(Campaign.creator), joinedload(Campaign.client)).where(Campaign.creator_id == user_id)
...
async def create_campaign(
    ...
    if viewerId is not None or adminId is not None:
        ...
        new_campaign = Campaign(
            ...
            creator_id=user_id,
            client_id=campaign_data.client_id,  # 클라이언트 ID 저장
